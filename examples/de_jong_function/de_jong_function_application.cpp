...
/*
   try
   {
      ...
      performance_functional.set_objective_term_pointer(de_Jong_function);
  
      Vector<double> initial_guess(variables_number, 1.0);

      independent_parameters_pointer->set_parameters(initial_guess);
 
      const double performance = performance_functional.calculate_evaluation();
      ...
      neural_network.save("../data/de_jong_function/neural_network.xml");

      training_strategy.save("../data/de_jong_function/training_strategy.xml");
      training_strategy_results.save("../data/de_jong_function/training_strategy_results.dat");
