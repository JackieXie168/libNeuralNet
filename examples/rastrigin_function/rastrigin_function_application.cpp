...
/*
   try
   {
      ...
      ipp->initialize_parameters_uniform(-5.12, 5.12);
      ...
      pf.set_objective_term_type(PerformanceFunctional::USER_PERFORMANCE_TERM);

      RastriginFunction* rf = new RastriginFunction(&nn);

      pf.set_objective_term_pointer(rf);

      // Training strategy

      TrainingStrategy ts(&pf);

      ts.set_initialization_training_algorithm_type(TrainingStrategy::EVOLUTIONARY_ALGORITHM);

      EvolutionaryAlgorithm* ea = new EvolutionaryAlgorithm(&pf);
      ea->initialize_population_uniform(-5.12, 5.12);

      ts.set_initialization_training_algorithm_pointer(ea);

      ts.set_initialization_training_algorithm_flag(true);

      ts.set_main_training_algorithm_type(TrainingStrategy::QUASI_NEWTON_METHOD);
