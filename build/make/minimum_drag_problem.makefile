...
SOURCE_FILES =  \
drag.cpp \
minimum_drag_problem_application.cpp
...
EXE_NAME = $(OBJECTS_DIR)/minimum_drag_problem.exe
...
all:
	$(MAKE) -f minimum_drag_problem.makefile deb

deb:
	+( make -f tinyxml.makefile)
	+( make -f opennn.makefile)
	$(MAKE) -f minimum_drag_problem.makefile exe

rel:
	+(make -f opennn.makefile rel)
	+(make -f tinyxml.makefile rel )
	$(MAKE) -f minimum_drag_problem.makefile exe "FLAGS=$(FLAGS_O)" "DESTINATION_DIR=release"
...
clean:
	...
	$(MAKE) -f minimum_drag_problem.makefile clean_rel "FLAGS=$(FLAGS_O)" "DESTINATION_DIR=release"
