...
#                                                                                                                #
#   C A T E N A R Y   P R O B L E M   M A K E F I L E                                                            #
#                                                                                                                #
...
catenary_problem/potential_energy.cpp \
catenary_problem_application.cpp \

INCLUDE_FILES = \
geodesic_problem/arc_length.h \
catenary_problem/potential_energy.h \
...
EXE_NAME = $(OBJECTS_DIR)/catenary_problem.exe
...
all:
	$(MAKE) -f catenary_problem.makefile deb

deb:
	+( make -f tinyxml.makefile)
	+( make -f opennn.makefile)
	$(MAKE) -f catenary_problem.makefile exe

rel:
	+(make -f opennn.makefile rel)
	+( cd "$(TINYXML_BUILD_DIR)"; make rel )
	$(MAKE) -f catenary_problem.makefile exe "FLAGS=$(FLAGS_O)" "DESTINATION_DIR=release"
...
clean:
	...
	$(MAKE) -f catenary_problem.makefile clean_rel "FLAGS=$(FLAGS_O)" "DESTINATION_DIR=release"
