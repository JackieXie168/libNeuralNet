...
#                                                                                                                #
#   C A R   P R O B L E M   N E U R O C O M P U T I N G   M A K E F I L E                                        #
#                                                                                                                #
...
car_problem_neurocomputing_application.cpp \
...
EXE_NAME = $(OBJECTS_DIR)/car_problem_neurocomputing.exe
...
all:
	$(MAKE) -f car_problem_neurocomputing.makefile deb

deb:
	+( make -f tinyxml.makefile)
	+( make -f opennn.makefile)
	$(MAKE) -f car_problem_neurocomputing.makefile exe

rel:
	+(make -f opennn.makefile rel)
	+( cd "$(TINYXML_BUILD_DIR)"; make rel )
	$(MAKE) -f car_problem_neurocomputing.makefile exe "FLAGS=$(FLAGS_O)" "DESTINATION_DIR=release"
...
clean:
	...
	$(MAKE) -f car_problem_neurocomputing.makefile clean_rel "FLAGS=$(FLAGS_O)" "DESTINATION_DIR=release"
